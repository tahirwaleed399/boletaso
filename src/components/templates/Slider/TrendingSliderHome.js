...
const date = new Date(item.date_time);
  ...
  return ( country && city && state && 
	<SliderWrapper >
							<SliderBox>
								<SliderImageBox>
									<SliderImageNumberBox>{`#${item.id + 1}`}</SliderImageNumberBox>
									<SliderImage
										src={item.image_1}
								...
								<SliderTextBox>
									<SliderTextSplitBox>
										...
										<SliderDetails>{formattedDate}</SliderDetails>
										<SliderDetails>{country.name} ,  {city.name} , {state.name}</SliderDetails>
									</SliderTextSplitBox>
